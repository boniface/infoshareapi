...
import com.outworkers.phantom.builder.QueryBuilder.Collections
...
class UserImageServiceTest extends FunSuite with BeforeAndAfter{
  ...
  before{
    userImgEntity = UserImages(org = "CPUT", userId = "test@email.com", id="2", description = "my pic",
                  url="http://www.cput.ac.za/logo.png", mime = ".png", size = None ,date = new Date())
    kwargs = Map("org"->userImgEntity.org,"userId"->userImgEntity.userId,"id"->userImgEntity.id)
  ...
  test("Get USER_IMAGE BY_ID"){
    ...
    assert(resp.head.userId equals userImgEntity.userId)
  ...
  test("Get ALL USER ORG_IMG"){
    updateEntity = userImgEntity.copy(id= "4",userId = "me2@test.com")
    val resp2 = Await.result(userImgService.save(updateEntity), 2.minutes)
